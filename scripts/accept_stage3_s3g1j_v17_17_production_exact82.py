...
EXPECTED_RECOVERIES = {"1225153907", "1212731093"}
...
V17_17_HEADER_SOURCE = "V17_17_STRICT_THREE_COLUMN_TWO_ROW_YEAR_MONTH_DAY_HEADER"
...
def download(session: requests.Session, url: str) -> bytes:
    response = session.get(
        url,
        headers={
            "User-Agent": "Mozilla/5.0 S3G1J-V17.17-production-exact82-acceptance",
...
def validate_v17_15(block: dict) -> None:
    if block.get("arbitration") != V17_15_ARBITRATION:
        raise ValueError(f"unexpected V17.15 arbitration {block.get('arbitration')}")
    if block.get("identity_tolerance") != "0.005":
        raise ValueError("V17.15 identity tolerance changed")
    if block.get("adjacent_row_bridge_selected_concepts") != ["TOTAL_EQUITY"]:
        raise ValueError(
            f"unexpected V17.15 bridge concepts {block.get('adjacent_row_bridge_selected_concepts')}"
        )
    if block.get("global_row_tolerance_changed") is not False:
        raise ValueError("V17.15 global row tolerance changed")


def validate_v17_17(block: dict, parsed: dict) -> None:
    if block.get("arbitration") != V17_17_ARBITRATION:
        raise ValueError(f"unexpected V17.17 arbitration {block.get('arbitration')}")
    if block.get("identity_tolerance") != "0.005":
        raise ValueError("V17.17 identity tolerance changed")
    if str(block.get("identity_residual_cny")) not in {"0", "0.0", "0.00"}:
        raise ValueError(f"V17.17 identity residual is not zero {block.get('identity_residual_cny')}")
    if block.get("adjacent_row_bridge_selected_concepts") != ["TOTAL_ASSETS", "TOTAL_LIABILITIES"]:
        raise ValueError(
            f"unexpected V17.17 bridge concepts {block.get('adjacent_row_bridge_selected_concepts')}"
        )
    if block.get("strict_total_equity_selected_concepts") != ["TOTAL_EQUITY"]:
        raise ValueError(
            f"unexpected V17.17 strict equity concepts {block.get('strict_total_equity_selected_concepts')}"
        )
    if block.get("strict_total_equity_alias") != "股东权益总计":
        raise ValueError(f"unexpected V17.17 strict equity alias {block.get('strict_total_equity_alias')}")
    if block.get("paired_header_evidence_source") != V17_17_HEADER_SOURCE:
        raise ValueError(f"unexpected V17.17 header source {block.get('paired_header_evidence_source')}")
    if block.get("paired_header_expected_column_index") != 0:
        raise ValueError("V17.17 frozen-date header did not select first column")
    if block.get("paired_header_column_count") != 3:
        raise ValueError("V17.17 paired header is not exact three-column evidence")
    if block.get("e_equals_a_minus_l_inference") is not False:
        raise ValueError("V17.17 equity was inferred instead of explicitly extracted")
    if block.get("global_row_tolerance_changed") is not False:
        raise ValueError("V17.17 global row tolerance changed")
    observations = parsed.get("observations") or {}
    expected_values = {
    ...
    if actual_values != expected_values:
        raise ValueError(f"unexpected V17.17 recovered values {actual_values}")
...
def main() -> int:
    ...
    if len(source_rows) != EXPECTED_TOTAL:
        raise ValueError(f"expected 82 accepted residual rows, got {len(source_rows)}")
    ...
    for index, row in enumerate(rows, 1):
        ...
        try:
            ...
            expected_found = aid in EXPECTED_RECOVERIES

            if expected_found:
                if not validated:
                    raise ValueError(
                        f"intended production recovery is not validated aid={aid} "
                        f"raw_found={raw_found} block={block} errors={errors}"
                    )
                if aid == V17_15_ID:
                    validate_v17_15(block)
                elif aid == V17_17_ID:
                    validate_v17_17(block, parsed)
            else:
                if validated or block is not None:
                    raise ValueError("unexpected validated production recovery outside exact-two set")
                if not errors:
        ...
        print(
            f"S3G1J_V17_17_PRODUCTION_EXACT82 shard={args.shard} {index}/{len(rows)} aid={aid}",
            flush=True,
        )
    ...
    report = {
        "gate": "S3G1J_V17_17_PRODUCTION_EXACT_82_ACCEPTANCE_SHARD",
