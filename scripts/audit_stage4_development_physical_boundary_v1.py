...
def main() -> int:
    ...
    try:
        ...
        start = basis["scope"]["development_start"]
        end = basis["scope"]["development_end"]
        package = Path(args.package_dir)

        source_verification = json.loads(Path(args.source_verification).read_text(encoding="utf-8"))
        check("source_verification_status", source_verification.get("status") == "VERIFIED")
        observed = source_verification.get("artifacts", {})
        ...
        g3 = package / outputs["g3"]
        g4 = package / outputs["g4"]
        oof = package / outputs["c007_oof"]
        ...
        for guard in [
            "all_output_min_dates_gte_development_start",
            "all_output_max_dates_lte_development_end",
            ...
            expected_value = guard in {
                "all_output_min_dates_gte_development_start",
                "all_output_max_dates_lte_development_end",
            }
            check(f"manifest_guard_{guard}", guards.get(guard) is expected_value)
        check("manifest_post_2022_zero", int(guards.get("post_2022_output_rows", -1)) == 0)
        ...
        check("oof_byte_identity", sha256_file(oof) == expected_oof_sha)

        con = duckdb.connect()
        con.execute("PRAGMA threads=2")
        g3_stats = row(con, f"SELECT count(*)::BIGINT rows,count(DISTINCT (trade_date,exchange,code))::BIGINT unique_keys,min(trade_date) date_min,max(trade_date) date_max,count(*) FILTER(WHERE trade_date<DATE {q(start)} OR trade_date>DATE {q(end)})::BIGINT outside_rows FROM read_parquet({q(str(g3))})")
        g4_stats = row(con, f"SELECT count(*)::BIGINT rows,count(DISTINCT (trade_date,exchange,code))::BIGINT unique_keys,min(trade_date) date_min,max(trade_date) date_max,count(*) FILTER(WHERE trade_date<DATE {q(start)} OR trade_date>DATE {q(end)})::BIGINT outside_rows FROM read_parquet({q(str(g4))})")
        oof_stats = row(con, f"SELECT count(*)::BIGINT rows,count(DISTINCT trade_date)::BIGINT decision_days,count(DISTINCT split_id)::BIGINT split_count,min(trade_date) date_min,max(trade_date) date_max,count(*) FILTER(WHERE trade_date<DATE {q(start)} OR trade_date>DATE {q(end)})::BIGINT outside_rows FROM read_parquet({q(str(oof))})")
        ...
        check("oof_rows", int(oof_stats["rows"]) == int(oof_expected["expected_rows"]))
        check("oof_decision_days", int(oof_stats["decision_days"]) == int(oof_expected["expected_decision_days"]))
        check("oof_split_count", int(oof_stats["split_count"]) == int(oof_expected["expected_split_count"]))

        universe_rows = int(con.execute(f"SELECT count(*) FROM read_parquet({q(str(g3))}) WHERE close>0 AND close<70").fetchone()[0])
        ...
        con.execute(f"""
          ...
          WITH d AS (SELECT DISTINCT trade_date FROM read_parquet({q(str(g3))}))
          SELECT trade_date, lead(trade_date) OVER(ORDER BY trade_date) next_trade_date FROM d
        """)
        structural = row(con, f"""
          WITH o AS (
            SELECT trade_date,upper(exchange) exchange,lpad(CAST(code AS VARCHAR),6,'0') code
            ...
            LEFT JOIN read_parquet({q(str(g3))}) d USING(trade_date,exchange,code)
            LEFT JOIN read_parquet({q(str(g4))}) gd USING(trade_date,exchange,code)
            LEFT JOIN read_parquet({q(str(g3))}) e ON cm.next_trade_date=e.trade_date AND o.exchange=e.exchange AND o.code=e.code
            LEFT JOIN read_parquet({q(str(g4))}) ge ON cm.next_trade_date=ge.trade_date AND o.exchange=ge.exchange AND o.code=ge.code
          )
