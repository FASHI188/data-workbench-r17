...
import extract_stage3_financial_pdf_values_v13 as prior
from stage3_financial_pdf_parser_v15 import parse_pdf_bytes as v17_24_parse_pdf_bytes

base = prior.base
METHOD = "CNINFO_ORIGINAL_PDF_PYMUPDF_V14_V17_24_EXACT_CORRUPTED_GROUP_EQUITY_ALIAS_FINAL_FALLBACK"
METHODOLOGY_VERSION = "V3.3.4-V17.24"
SHARD_GATE = "S3G1J_FINANCIAL_PDF_EXTRACTION_SHARD_V17_24"
...
def parse_pdf_bytes(raw: bytes, economic_date: str) -> dict:
    parsed = dict(v17_24_parse_pdf_bytes(raw, economic_date))
    parsed["declared_a_share_codes"] = prior.prior.v15.v14.v9.declared_a_share_codes(raw)
    return parsed
...
def _rewrite_v17_24_shard_manifest(out_root: str, shard: int) -> None:
    ...
    if manifest.get("parser_method") != METHOD:
        raise ValueError(
            f"V17.24 shard parser method mismatch: {manifest.get('parser_method')}"
        )
    if manifest.get("methodology_version") != METHODOLOGY_VERSION:
        raise ValueError(
            f"V17.24 shard methodology mismatch: {manifest.get('methodology_version')}"
        )
    if int(manifest.get("shard", -1)) != shard:
        raise ValueError(
            f"V17.24 shard identity mismatch expected={shard} "
            f"actual={manifest.get('shard')}"
        )
    manifest["gate"] = SHARD_GATE
    manifest["runtime_generation"] = "V17.24"
...
def main() -> int:
    ...
    _rewrite_v17_24_shard_manifest(out_root, shard)
    return result
