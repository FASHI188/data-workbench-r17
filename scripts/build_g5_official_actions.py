...
def req(session,url,params,headers,attempts=5,timeout=45):
 last=None
 for i in range(attempts):
  try:
   r=session.get(url,params=params,headers=headers,timeout=timeout) if headers.get('_method')=='GET' else session.post(url,params=params,headers={k:v for k,v in headers.items() if k!='_method'},timeout=timeout)
   r.raise_for_status()
...
def sse_get(session,sql_id,extra):
 ...
 raw,final=req(session,url,params,{'User-Agent':UA,'Referer':'https://www.sse.com.cn/','Accept':'*/*','X-Requested-With':'XMLHttpRequest','_method':'GET'})
...
def build_sse(out):
 ...
 for y in range(2015,2027):
  specs=[
   ('DIVIDEND','COMMON_SSE_SJ_GPSJ_FHSG_SSGSFHQK_L',{'CONDITION_AG':'1','A_REG_DATE':str(y),'A_STOCK_CODE':''}),
   ('BONUS','COMMON_SSE_SJ_GPSJ_FHSG_SG_L',{'SEARCH_YEAR':str(y),'COMPANY_CODE':''}),
   ('RIGHTS','COMMON_SSE_SJ_GPSJ_MJZJ_PG_ZBAKCB_L',{'SEARCH_YEAR':str(y),'LIST_BOARD':'1'}),
  ]
   ...
   try:
    ...
    for x in src:
     if component=='DIVIDEND':
      code=clean(x.get('A_STOCK_CODE'));ex=norm_date(x.get('A_DIV_DATE'));record=norm_date(x.get('A_REG_DATE'))
      vals={'cash_per_share':str(num(x.get('A_BEFR_TAX_DIV'))),'bonus_per_share':'0','transfer_per_share':'0','rights_per_share':'0','rights_price':'0','rights_listing_date':''}
     elif component=='BONUS':
      code=clean(x.get('A_STOCK_CODE'));ex=norm_date(x.get('A_DERIGHTS_DATE'));record=norm_date(x.get('A_REG_DATE'))
      vals={'cash_per_share':'0','bonus_per_share':str(num(x.get('BONUS_RATIO'))/Decimal(10)),'transfer_per_share':str(num(x.get('CONVERT_RATIO'))/Decimal(10)),'rights_per_share':'0','rights_price':'0','rights_listing_date':''}
     else:
      code=clean(x.get('COMPANY_CODE'));ex=norm_date(x.get('DERIGHTS_DATE'));record=norm_date(x.get('REG_DATE'))
      vals={'cash_per_share':'0','bonus_per_share':'0','transfer_per_share':'0','rights_per_share':str(num(x.get('RIGHTS_RATIO'))/Decimal(10)),'rights_price':str(num(x.get('RIGHTS_PRICE'))),'rights_listing_date':norm_date(x.get('RIGHTS_LIST_DATE'))}
...
def init_cninfo():
 ...
 js=py_mini_racer.MiniRacer();js.eval(Path(get_ths_js('cninfo.js')).read_text(encoding='utf-8'))
 return js
...
def cn_headers(js,origin='https://webapi.cninfo.com.cn'):
 return {'Accept':'*/*','Accept-Enckey':js.call('getResCode1'),'Accept-Encoding':'gzip, deflate','Accept-Language':'zh-CN,zh;q=0.9,en;q=0.8','Cache-Control':'no-cache','Content-Length':'0','Host':'webapi.cninfo.com.cn','Origin':origin,'Pragma':'no-cache','Referer':origin+'/','User-Agent':UA,'X-Requested-With':'XMLHttpRequest'}
