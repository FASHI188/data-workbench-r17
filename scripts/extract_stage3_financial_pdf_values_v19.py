...
import extract_stage3_financial_pdf_values_v18 as prior
from extract_stage3_financial_pdf_values_v6 import declared_a_share_codes
from stage3_financial_pdf_parser_v21 import parse_pdf_bytes as v17_29_parse_pdf_bytes

base = prior.base
METHOD = (
    "CNINFO_ORIGINAL_PDF_PYMUPDF_V19_V17_29_"
    "EXACT_SOURCE_SPLIT_GROUP_EQUITY_PRODUCTION"
)
METHODOLOGY_VERSION = "V3.3.13-V17.29"
SHARD_GATE = "S3G1J_FINANCIAL_PDF_EXTRACTION_SHARD_V17_29"
RUNTIME_GENERATION = "V17.29"
...
def parse_pdf_bytes(raw: bytes, economic_date: str) -> dict:
    parsed = dict(v17_29_parse_pdf_bytes(raw, economic_date))
...
def _rewrite_manifest(out_root: str, shard: int) -> None:
    ...
    if manifest.get("parser_method") != METHOD:
        raise ValueError(
            f"V17.29 shard parser method mismatch: {manifest.get('parser_method')}"
        )
    if manifest.get("methodology_version") != METHODOLOGY_VERSION:
        raise ValueError(
            f"V17.29 shard methodology mismatch: {manifest.get('methodology_version')}"
        )
    if int(manifest.get("shard", -1)) != shard:
        raise ValueError(
            f"V17.29 shard identity mismatch expected={shard} "
