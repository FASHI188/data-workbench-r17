...
def synthetic()->int:
    assert (1==0 or 1==0 or 0<=0 or False) is True
    assert (1==0 or 1==0 or 100<=0 or False) is False
    print(json.dumps({'runtime_veto_independent_synthetic_self_test':'PASS','outcome_read':False,'backfill':False}))
    return 0
...
def main()->int:
    ...
    try:
        ...
        source_mismatch=con.execute("""SELECT count(*) FROM r x
          ...
          WHERE x.decision_close IS DISTINCT FROM dm.close OR x.decision_tradable IS DISTINCT FROM ds.tradable OR x.decision_risk_warning IS DISTINCT FROM ds.risk_warning
             ...
             OR x.entry_tradable IS DISTINCT FROM es.tradable OR x.entry_risk_warning IS DISTINCT FROM es.risk_warning OR x.entry_preclose IS DISTINCT FROM es.preclose OR x.entry_limit_up_rate IS DISTINCT FROM es.limit_up_rate OR x.entry_limit_down_rate IS DISTINCT FROM es.limit_down_rate
             ...
             OR x.exit_tradable IS DISTINCT FROM xs.tradable OR x.exit_risk_warning IS DISTINCT FROM xs.risk_warning OR x.exit_preclose IS DISTINCT FROM xs.preclose OR x.exit_limit_up_rate IS DISTINCT FROM xs.limit_up_rate OR x.exit_limit_down_rate IS DISTINCT FROM xs.limit_down_rate""").fetchone()[0]; ck('raw_state_matches_physical_sources',int(source_mismatch)==0,str(source_mismatch))
        flag_mismatch=con.execute("""SELECT count(*) FROM r WHERE
          decision_signal_invalid != CAST(decision_tradable=0 AS INTEGER)
          OR entry_one_price != CAST(abs(entry_high-entry_low)<=1e-12 AS INTEGER)
          OR entry_one_price_limit_up != CAST(abs(entry_high-entry_low)<=1e-12 AND entry_volume_shares>0 AND abs(entry_close-floor(entry_preclose*(1.0+entry_limit_up_rate)*100.0+0.5)/100.0)<=0.005001 AS INTEGER)
          OR entry_one_price_limit_down != CAST(abs(entry_high-entry_low)<=1e-12 AND entry_volume_shares>0 AND abs(entry_close-floor(entry_preclose*(1.0-entry_limit_down_rate)*100.0+0.5)/100.0)<=0.005001 AS INTEGER)
          OR hard_entry_veto != CAST((entry_tradable=0) OR (entry_volume_shares<=0) OR (abs(entry_high-entry_low)<=1e-12 AND entry_volume_shares>0 AND abs(entry_close-floor(entry_preclose*(1.0+entry_limit_up_rate)*100.0+0.5)/100.0)<=0.005001) AS INTEGER)
          OR exit_one_price != CAST(abs(exit_high-exit_low)<=1e-12 AS INTEGER)
          OR exit_one_price_limit_down != CAST(abs(exit_high-exit_low)<=1e-12 AND exit_volume_shares>0 AND abs(exit_close-floor(exit_preclose*(1.0-exit_limit_down_rate)*100.0+0.5)/100.0)<=0.005001 AS INTEGER)
          OR exit_one_price_limit_up != CAST(abs(exit_high-exit_low)<=1e-12 AND exit_volume_shares>0 AND abs(exit_close-floor(exit_preclose*(1.0+exit_limit_up_rate)*100.0+0.5)/100.0)<=0.005001 AS INTEGER)
          OR hard_exit_veto != CAST((exit_tradable=0) OR (exit_volume_shares<=0) OR (abs(exit_high-exit_low)<=1e-12 AND exit_volume_shares>0 AND abs(exit_close-floor(exit_preclose*(1.0-exit_limit_down_rate)*100.0+0.5)/100.0)<=0.005001) AS INTEGER)
          OR runtime_hard_veto != CAST((decision_tradable=0) OR (entry_tradable=0) OR (entry_volume_shares<=0) OR (abs(entry_high-entry_low)<=1e-12 AND entry_volume_shares>0 AND abs(entry_close-floor(entry_preclose*(1.0+entry_limit_up_rate)*100.0+0.5)/100.0)<=0.005001) OR (exit_tradable=0) OR (exit_volume_shares<=0) OR (abs(exit_high-exit_low)<=1e-12 AND exit_volume_shares>0 AND abs(exit_close-floor(exit_preclose*(1.0-exit_limit_down_rate)*100.0+0.5)/100.0)<=0.005001) AS INTEGER)
        ...
        result={'schema_version':1,'status':'PASS' if not failures else 'FAIL','pass':not failures,'runtime_veto_contract_fingerprint':RUNTIME_FP,'physical_boundary_contract_fingerprint':BOUNDARY_FP,'failed_checks':failures,'checks':checks,'recomputed_coverages':recomputed,'recomputed_gate_checks':gates,'recomputed_gate_pass':all(gates.values()),'oos_outcome_values_read':False,'predictions_recomputed':False,'model_loaded':False,'fit_retrain_tune_reselect_executed':False,'backfill_performed':False,'replacement_performed':False,'post_selection_drop_performed':False,'final_lockbox_accessed':False}
    except Exception as e:
        failures.append(f'exception: {type(e).__name__}: {e}'); result={'schema_version':1,'status':'FAIL','pass':False,'failed_checks':failures,'checks':checks}
