...
def _qualified_header_row(row: dict, expected: str, alias_x1: float) -> tuple[list[dict], int] | None:
    ...
    if any(token in compact for token in HEADER_BLOCKERS):
        return None
    expected_cn = expected.replace("-", "年", 1)
    # Convert 2024-12-31 -> 2024年12月31日 for exact single-date headers.
...
def column_role_evidence(
    ...
    pass_gate = abs(float(expected_amount["x0"]) - selected_x) <= X_TOLERANCE
    return {
        "pass": pass_gate,
        "reason": None if pass_gate else "selected value is not the frozen-date ordinal amount",
        "header": header,
        "amounts": [{"raw": str(a["raw"]), "value": str(a["value"]), "x0": float(a["x0"])} for a in amounts],
        "expected_amount": {
...
def diagnose_spatial_balance_sheet_v16_7(
    ...
    out["column_role_gate"] = {
        "pass": all_pass,
        "concepts": evidence,
        "policy": "frozen economic-date ordinal must match the selected amount column",
    }
