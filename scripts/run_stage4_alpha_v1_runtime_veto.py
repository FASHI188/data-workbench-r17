...
def synthetic()->int:
    ...
    rows=[{'decision_tradable':1,'entry_tradable':1,'entry_volume':100,'entry_one_up':0,'exit_tradable':1,'exit_volume':100,'exit_one_down':0},
          {'decision_tradable':1,'entry_tradable':1,'entry_volume':100,'entry_one_up':1,'exit_tradable':1,'exit_volume':100,'exit_one_down':0}]
    veto=[(r['decision_tradable']==0 or r['entry_tradable']==0 or r['entry_volume']<=0 or r['entry_one_up']==1 or r['exit_tradable']==0 or r['exit_volume']<=0 or r['exit_one_down']==1) for r in rows]
    assert veto==[False,True]
    print(json.dumps({'runtime_veto_synthetic_self_test':'PASS','backfill':False,'replacement':False,'risk_warning_hard_veto':False}))
    return 0
...
def main()->int:
    ...
    ps=one(con,f"SELECT count(*)::BIGINT rows,count(DISTINCT (trade_date,exchange,code))::BIGINT unique_keys,min(trade_date) min_date,max(trade_date) max_date,count(*) FILTER(WHERE prediction IS NULL OR NOT isfinite(prediction))::BIGINT invalid_predictions FROM p")
    ...
    con.execute(f"""CREATE TEMP TABLE ranked AS
      SELECT p.*,row_number() OVER(PARTITION BY trade_date ORDER BY prediction DESC,exchange ASC,code ASC) pred_rank,count(*) OVER(PARTITION BY trade_date) date_n,
             row_number() OVER(ORDER BY trade_date,exchange,code) global_row
    ...
    con.execute("""CREATE TEMP TABLE joined AS
      SELECT x.*,
        dm.close decision_close,ds.tradable decision_tradable,ds.risk_warning decision_risk_warning,
        em.high entry_high,em.low entry_low,em.close entry_close,em.volume_shares entry_volume_shares,
        es.tradable entry_tradable,es.risk_warning entry_risk_warning,es.preclose entry_preclose,es.limit_up_rate entry_limit_up_rate,es.limit_down_rate entry_limit_down_rate,
        xm.high exit_high,xm.low exit_low,xm.close exit_close,xm.volume_shares exit_volume_shares,
        xs.tradable exit_tradable,xs.risk_warning exit_risk_warning,xs.preclose exit_preclose,xs.limit_up_rate exit_limit_up_rate,xs.limit_down_rate exit_limit_down_rate
      FROM schedule x
    ...
    missing=one(con,"""SELECT count(*) FILTER(WHERE entry_date IS NULL OR exit_date IS NULL)::BIGINT missing_schedule,
      count(*) FILTER(WHERE decision_close IS NULL OR decision_tradable IS NULL OR decision_risk_warning IS NULL)::BIGINT missing_decision_state,
      count(*) FILTER(WHERE entry_high IS NULL OR entry_low IS NULL OR entry_close IS NULL OR entry_volume_shares IS NULL OR entry_tradable IS NULL OR entry_risk_warning IS NULL OR entry_preclose IS NULL OR entry_limit_up_rate IS NULL OR entry_limit_down_rate IS NULL)::BIGINT missing_entry_state,
      count(*) FILTER(WHERE exit_high IS NULL OR exit_low IS NULL OR exit_close IS NULL OR exit_volume_shares IS NULL OR exit_tradable IS NULL OR exit_risk_warning IS NULL OR exit_preclose IS NULL OR exit_limit_up_rate IS NULL OR exit_limit_down_rate IS NULL)::BIGINT missing_exit_state FROM joined""")
    if any(int(v)!=0 for v in missing.values()): raise ValueError(f'missing required execution state: {missing}')
    con.execute("""CREATE TEMP TABLE flags AS SELECT *,
      (decision_close<2.0)::INTEGER decision_low_price_lt2,(decision_tradable=0)::INTEGER decision_signal_invalid,
      (abs(entry_high-entry_low)<=1e-12)::INTEGER entry_one_price,
      (abs(entry_high-entry_low)<=1e-12 AND entry_volume_shares>0 AND abs(entry_close-floor(entry_preclose*(1.0+entry_limit_up_rate)*100.0+0.5)/100.0)<=0.005001)::INTEGER entry_one_price_limit_up,
      (abs(entry_high-entry_low)<=1e-12 AND entry_volume_shares>0 AND abs(entry_close-floor(entry_preclose*(1.0-entry_limit_down_rate)*100.0+0.5)/100.0)<=0.005001)::INTEGER entry_one_price_limit_down,
      ((entry_tradable=0) OR (entry_volume_shares<=0) OR (abs(entry_high-entry_low)<=1e-12 AND entry_volume_shares>0 AND abs(entry_close-floor(entry_preclose*(1.0+entry_limit_up_rate)*100.0+0.5)/100.0)<=0.005001))::INTEGER hard_entry_veto,
      (abs(exit_high-exit_low)<=1e-12)::INTEGER exit_one_price,
      (abs(exit_high-exit_low)<=1e-12 AND exit_volume_shares>0 AND abs(exit_close-floor(exit_preclose*(1.0-exit_limit_down_rate)*100.0+0.5)/100.0)<=0.005001)::INTEGER exit_one_price_limit_down,
      (abs(exit_high-exit_low)<=1e-12 AND exit_volume_shares>0 AND abs(exit_close-floor(exit_preclose*(1.0+exit_limit_up_rate)*100.0+0.5)/100.0)<=0.005001)::INTEGER exit_one_price_limit_up,
      ((exit_tradable=0) OR (exit_volume_shares<=0) OR (abs(exit_high-exit_low)<=1e-12 AND exit_volume_shares>0 AND abs(exit_close-floor(exit_preclose*(1.0-exit_limit_down_rate)*100.0+0.5)/100.0)<=0.005001))::INTEGER hard_exit_veto,
      ((decision_tradable=0) OR (entry_tradable=0) OR (entry_volume_shares<=0) OR (abs(entry_high-entry_low)<=1e-12 AND entry_volume_shares>0 AND abs(entry_close-floor(entry_preclose*(1.0+entry_limit_up_rate)*100.0+0.5)/100.0)<=0.005001) OR (exit_tradable=0) OR (exit_volume_shares<=0) OR (abs(exit_high-exit_low)<=1e-12 AND exit_volume_shares>0 AND abs(exit_close-floor(exit_preclose*(1.0-exit_limit_down_rate)*100.0+0.5)/100.0)<=0.005001))::INTEGER runtime_hard_veto
      FROM joined""")
    ...
    summary={'schema_version':1,'status':'PASS' if gate_pass else 'FAIL','runtime_veto_contract_fingerprint':RUNTIME_FP,'physical_boundary_contract_fingerprint':BOUNDARY_FP,'selection_population':'FROZEN_OOS_PREDICTIONS_NO_SCORE_FILTER','rebalance_dates':rebalances,'backfill_performed':False,'replacement_performed':False,'post_selection_drop_performed':False,'oos_outcome_values_read':False,'risk_warning_hard_veto':False,'decision_low_price_lt2_hard_veto':False,'missing_required_execution_state':missing,'coverages':summaries,'gate_checks':gate_checks,'gate_pass':gate_pass,'alpha_oos_gate_modified':False,'runtime_veto_cannot_rescue_alpha_failure':True,'failure_action':'NO_PROMOTION_NO_BACKFILL_NO_RETUNING_ON_OOS'}
