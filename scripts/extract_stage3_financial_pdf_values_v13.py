...
SHARD_GATE = "S3G1J_FINANCIAL_PDF_EXTRACTION_SHARD_V17_21"
...
def _rewrite_v17_21_shard_manifest(out_root: str, shard: int) -> None:
    ...
    if manifest.get("parser_method") != METHOD:
        raise ValueError(
            f"V17.21 shard parser method mismatch: {manifest.get('parser_method')}"
        )
    if manifest.get("methodology_version") != METHODOLOGY_VERSION:
        raise ValueError(
            "V17.21 shard methodology mismatch: "
            f"{manifest.get('methodology_version')}"
        )
    if int(manifest.get("shard", -1)) != shard:
        raise ValueError(
            f"V17.21 shard manifest identity mismatch expected={shard} "
            f"actual={manifest.get('shard')}"
        )

    # The reused V17.17 driver hard-codes its historical gate label. Replace only
    # that metadata after all files and digests have been written; numeric rows
    # already carry the V17.21 method and methodology through the patched globals.
    manifest["gate"] = SHARD_GATE
    manifest["runtime_generation"] = "V17.21"
...
def main() -> int:
    ...
    _rewrite_v17_21_shard_manifest(out_root, shard)
    return result
