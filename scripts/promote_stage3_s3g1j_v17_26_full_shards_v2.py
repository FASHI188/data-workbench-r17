...
def main() -> int:
    ...
    source_execution_error_total = 0
    ...
    for manifest_path in manifests:
        ...
        if manifest.get("pass") is not True:
            raise ValueError(f"source shard execution did not pass {shard}")
        execution_error_count = int(manifest.get("error_count", -1))
        execution_errors = list(manifest.get("errors") or [])
        if execution_error_count != 0 or execution_errors:
            raise ValueError(
                f"source shard execution errors changed {shard}: "
                f"count={execution_error_count} errors={execution_errors[:5]}"
            )
        source_execution_error_total += execution_error_count
        ...
        manifest["source_execution_error_count"] = execution_error_count
        manifest["document_error_count"] = document_errors
        ...
        promoted_document_error_total += document_errors
    ...
    if source_execution_error_total != 0:
        raise ValueError(
            f"source execution error total changed {source_execution_error_total}"
        )
    ...
    report = {
        "gate": "S3G1J_V17_26_FULL_SHARD_EVIDENCE_PROMOTION_V2",
        "pass": True,
        ...
        "source_execution_errors": source_execution_error_total,
        "document_errors": promoted_document_error_total,
