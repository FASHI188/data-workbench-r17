...
def main() -> int:
    ...
    try:
        for sid in range(1,6):
            ...
            tbl=pa.table({'trade_date':pa.array(dates),'exchange':pa.array(exchanges.tolist()),'code':pa.array(codes.tolist()),'split_id':pa.array(np.full(len(pred),sid,dtype=np.int8)),'prediction':pa.array(pred),'excess_return_20d':pa.array(yte),'excess_return_5d':pa.array(y5),'stock_total_return_20d':pa.array(stock20),'benchmark_return_20d':pa.array(bench20)}); writer=pq.ParquetWriter(oof_path,tbl.schema,compression='zstd') if writer is None else writer; writer.write_table(tbl); del Xtr,Xte,ytr,yte,pred,dates,exchanges,codes,y5,stock20,bench20,model,tbl; gc.collect()
    finally:
