...
def main() -> int:
    ...
    def feature_sql(s: dict, ridge: bool) -> tuple[list[str],list[str]]:
        ...
        for c in features:
            if c=='regime_state':
                ...
                st=s['continuous_train_stats'][c]; lo=st['q001']; hi=st['q999']; med=st['median']; e=xbase(c)
    ...
    def table_to_xy(table: pa.Table, feature_names: list[str], include_keys: bool):
        ...
        for j,nm in enumerate(feature_names):
            arr=table.column(nm).combine_chunks().to_numpy(zero_copy_only=False); X[:,j]=np.asarray(arr,dtype=np.float32)
        ...
        for sid in range(1,6):
            ...
            try:
                with warnings.catch_warnings(record=True) as ws:
                    ...
                    model.fit(Xtr,ytr); pred=np.asarray(model.predict(Xte),dtype=np.float64); warn=[f"{w.category.__name__}:{w.message}" for w in ws]
            ...
            trial_log.append(trial); preprocess_manifest['splits'].append({'split_id':sid,'regime_levels_train':s['regime_levels_train'],'continuous_stats_sha256':s['stats_sha256'],'ridge_scaler':scaler_info,'model_input_feature_names':names,'model_input_feature_count':len(names)})
            if fit_error is not None:
                del Xtr,Xte,ytr,yte,dates,exchanges,codes,y5,stock20,bench20,model; gc.collect(); continue
            ...
            del Xtr,Xte,ytr,yte,pred,dates,exchanges,codes,y5,stock20,bench20,model,tbl; gc.collect()
    finally:
        if writer is not None: writer.close()

    candidate_valid=len(trial_log)==5 and all(t['status']=='SUCCESS' for t in trial_log) and len(split_metrics)==5 and all(s['split_valid'] for s in split_metrics); means=[s['mean_daily_ic_20d'] for s in split_metrics if s['mean_daily_ic_20d'] is not None]
    primary=float(np.median(means)) if candidate_valid else None; worst=float(np.min(means)) if candidate_valid else None; complexity=0 if family=='RIDGE_V1' else (10 if params['max_leaf_nodes']==15 else 20)
    ...
    pq.write_table(pa.Table.from_pylist(daily_rows),out/'daily_metrics.parquet',compression='zstd'); hashes={p.name:sha256_file(p) for p in out.iterdir() if p.is_file()}; (out/'artifact_hashes.json').write_text(json.dumps(hashes,sort_keys=True,indent=2)+'\n',encoding='utf-8')
