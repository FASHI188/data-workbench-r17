...
def main()->int:
    ...
    if s3gu.get('strict_price_rule')!='<70 CNY' or s3gu.get('exact_70_excluded') is not True:errors.append('S3GU strict price audit mismatch')
    ...
    dataset_shas={'periodic_filing_ledger':s3g1e.get('ledger_sha256'),'financial_raw_values':s3g1j.get('financial_values_sha256'),'financial_documents':s3g1j.get('financial_documents_sha256'),'announcement_ledger':s3g2.get('ledger_sha256'),'industry_ledger':s3g3.get('ledger_sha256'),'earnings_surprise':s3g4.get('surprise_ledger_sha256')}
    if any(not v for v in dataset_shas.values()):errors.append(f'final Stage3 dataset SHAs incomplete: {dataset_shas}')
    basis={'stage2_version':stage2.get('version'),'stage2_fingerprint':stage2.get('stage2_dataset_fingerprint'),'stage3_lock_version':lock.get('version'),'gate_artifact_digests':{k:digests.get(v.get('artifact')) for k,v in sorted(gates.items())},'dataset_shas':dataset_shas,'trading_universe_policy_sha256':sha_file(Path(a.universe_policy)),'scope':'SSE_MAIN_A + SZSE_MAIN_A; tradable candidate price strictly below CNY 70 point-in-time'}
