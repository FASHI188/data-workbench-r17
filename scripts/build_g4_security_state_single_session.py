...
def delisting_first_day(rows,listed_to):
 if not listed_to or not rows:return None
 # Look only near termination; consolidation follows a multi-day suspension and ends shortly before delisting.
 near=[r for r in rows if 0 < (listed_to-date.fromisoformat(r['date'])).days <= 90]
 if not near:return None
 last_trade=max((i for i,r in enumerate(near) if r['tradestatus']=='1'),default=-1)
 if last_trade<0:return None
 # Find start of the final tradable run, tolerating isolated in-period suspensions by walking 40 rows back.
 lo=max(0,last_trade-45); candidate=None
 for i in range(lo,last_trade+1):
  if near[i]['tradestatus']!='1':continue
  prior=near[max(0,i-8):i]
  if len(prior)>=5 and sum(x['tradestatus']=='0' for x in prior)>=5:
   later_trades=sum(x['tradestatus']=='1' for x in near[i:last_trade+1])
   if 1<=later_trades<=30:candidate=near[i]['date'];break
 return candidate

def base_rule(r,listed_from,listed_to,trade_index,delist_first):
 d=date.fromisoformat(r['date']);trad=r['tradestatus']=='1';st=r['isST']=='1'
 if not trad:return ('SUSPENDED','','','BAOSTOCK_POINT_IN_TIME')
 if delist_first and r['date']==delist_first:return ('DELISTING_CONSOLIDATION_FIRST_DAY_NO_LIMIT','','','BAOSTOCK_POINT_IN_TIME+EXCHANGE_RULE_VERSION+DELISTING_RESUME_PATTERN')
 if listed_from>=REGISTRATION_MAIN and trade_index<=5:return ('IPO_FIRST5_NO_LIMIT','','','LIFECYCLE+EXCHANGE_RULE_VERSION')
 if date(2014,1,1)<=listed_from<REGISTRATION_MAIN and trade_index==1:return ('IPO_FIRST_DAY_2014_RULE','0.44','0.36','LIFECYCLE+EXCHANGE_RULE_VERSION')
...
def main():
 ...
 files=[];writers=[];handles=[];man=[{'shard':i,'shards':SHARDS,'counts':{'securities':0,'rows':0,'tradable':0,'suspended':0,'risk_warning':0},'source_manifest':[],'zero_source_securities':[],'unclassified_special_days':[],'delisting_first_days':[]} for i in range(SHARDS)]
 try:
  ...
  try:
   for idx,(meta,a,b) in enumerate(read_intervals()):
    ...
    dfirst=delisting_first_day(rows,b)
    if dfirst:m['delisting_first_days'].append({'exchange':meta['exchange'],'code':meta['code'],'date':dfirst})
    ti=0
    for r in rows:
     ...
     lr,up,dn,evidence=base_rule(r,a,b,ti,dfirst)
 ...
 print(json.dumps({'securities':sum(x['counts']['securities'] for x in man),'rows':sum(x['counts']['rows'] for x in man),'zero':sum(len(x['zero_source_securities']) for x in man),'unclassified':sum(len(x['unclassified_special_days']) for x in man),'delisting_first_days':sum(len(x['delisting_first_days']) for x in man)},ensure_ascii=False))
 return 0
