...
def download(session: requests.Session, url: str) -> bytes:
    response = session.get(
        url,
        headers={
            "User-Agent": "Mozilla/5.0 S3G1J-V17.17-exact-81-safety-replay",
...
def main() -> int:
    ...
    if not accepted.get("pass") or len(accepted_rows) != 82:
    ...
    if len(remaining_ids) != EXPECTED_TOTAL or not remaining_ids.issubset(accepted_rows):
    ...
    for index, row in enumerate(rows, 1):
        ...
        try:
            ...
            if digest != accepted_rows[aid]["sha256"]:
                raise ValueError(
                    f"source SHA changed expected={accepted_rows[aid]['sha256']} actual={digest}"
                )
            with _mupdf_diagnostic_guard():
                with fitz.open(stream=raw, filetype="pdf") as doc:
                    baseline = v1715.diagnose_spatial_balance_sheet_v17_15(doc, row["economic_date"])
                    candidate = v1717.diagnose_spatial_balance_sheet_v17_17(doc, row["economic_date"])
            if baseline.get("recovered"):
                raise ValueError("accepted V17.15 residual unexpectedly recovered under baseline")
            ...
            strict_selected = sorted(
                ...
                if value.get("strict_same_row_equity_total")
            )
            if recovered:
                ...
                if strict_selected != ["TOTAL_EQUITY"]:
                    raise ValueError(f"unexpected strict selection {strict_selected}")
                equity = selected.get("TOTAL_EQUITY") or {}
                if equity.get("alias") != "股东权益总计":
                    raise ValueError(f"unexpected equity alias {equity.get('alias')}")
            results.append({
                "announcement_id": aid,
                ...
                "strict_candidate_counts": candidate.get("strict_candidate_counts"),
                "strict_equity_funnel": candidate.get("strict_equity_funnel"),
                "strict_selected_concepts": strict_selected,
        ...
        print(
            f"S3G1J_V17_17_EXACT81 shard={args.shard} {index}/{len(rows)} aid={aid}",
            flush=True,
        )

    recovered_ids = sorted(row["announcement_id"] for row in results if row["candidate_recovered"])
    report = {
        "gate": "S3G1J_V17_17_EXACT_81_SAFETY_REPLAY_SHARD",
