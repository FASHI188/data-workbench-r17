...
def main() -> int:
    ...
    ap.add_argument("--v17-7-acceptance", required=True)
    ...
    accepted = json.loads(Path(args.v17_7_acceptance).read_text(encoding="utf-8"))
    if not accepted.get("pass"):
        raise ValueError("V17.7 acceptance is not PASS")
    if int(accepted.get("v14_remaining_count", -1)) != 113:
        raise ValueError("V17.7 acceptance lost the frozen V14=113 baseline")
    if int(accepted.get("v17_7_recovery_count", -1)) != 25:
        raise ValueError("V17.7 accepted recovery count is not 25")
    if int(accepted.get("v17_7_remaining_count", -1)) != EXPECTED_TOTAL_REMAINING:
        raise ValueError("V17.7 accepted remaining count is not 88")
    if not accepted.get("all_v14_success_paths_unchanged"):
        raise ValueError("V14 success paths are not accepted unchanged")
    if not accepted.get("all_prior_v16_recoveries_preserved"):
        raise ValueError("prior V16.7 recoveries are not accepted preserved")
    if not accepted.get("all_recoveries_have_period_column_identity_evidence"):
        raise ValueError("V17.7 hard recovery evidence is incomplete")
    ...
    if len(target_ids) != EXPECTED_TOTAL_REMAINING:
        raise ValueError(f"expected 88 unique remaining ids, got {len(target_ids)}")
    ...
    for idx, row in enumerate(rows, 1):
        ...
        try:
            ...
            if diag.get("recovered"):
                raise AssertionError("accepted V17.7 remaining PDF unexpectedly recovers under identical parser")
        ...
        print(f"V17_8_RESIDUAL_FUNNEL shard={args.shard} {idx}/{len(rows)} aid={aid}", flush=True)

    report = {
        "gate": "S3G1J_V17_8_RESIDUAL_FUNNEL_SHARD",
        "shard": args.shard,
        "shards": args.shards,
