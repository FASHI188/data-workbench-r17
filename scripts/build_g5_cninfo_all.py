...
def allotment_fields(arr):
    if isinstance(arr,dict):
        return {
            '证券代码':arr.get('SECCODE'),'除权基准日':arr.get('F012D'),'股权登记日':arr.get('F011D'),
            '公告日期':arr.get('DECLAREDATE'),'配股比例':arr.get('F004N'),'配股价格':arr.get('F005N'),
            '配股上市日':arr.get('F038D'),'股票类别':arr.get('F002V'),'股票类别编码':arr.get('F001V'),
        }
    if isinstance(arr,list) and len(arr)==len(b.CNINFO_ALLOT_COLS):
        return dict(zip(b.CNINFO_ALLOT_COLS,arr))
    return None
