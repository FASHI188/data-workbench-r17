...
def detect_period(rows,listed_to):
 ...
 near=[r for r in rows if 0 < (listed_to-date.fromisoformat(r['trade_date'])).days <=120]
 trades=[i for i,r in enumerate(near) if r['tradable']=='1']
 ...
 return None
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--input',required=True);ap.add_argument('--out',required=True);a=ap.parse_args();src=Path(a.input);out=Path(a.out);out.mkdir(parents=True,exist_ok=True);ends=lifecycle_end();summary={'periods':[],'rewritten_rows':0}
 for sid in range(16):
  ...
  for k,rs in by.items():
   ...
   for r in rs:
    ...
    if p['regime']=='NEW_15DAY_FIRST_NO_LIMIT' and r['trade_date']==p['first_date']:
     target=('DELISTING_15DAY_FIRST_DAY_NO_LIMIT','','','BAOSTOCK_POINT_IN_TIME+EXCHANGE_2020_DELISTING_REFORM+OFFLINE_FINAL_BLOCK_RECLASS')
    else:
     target=('DELISTING_CONSOLIDATION_10PCT','0.10','0.10','BAOSTOCK_POINT_IN_TIME+EXCHANGE_DELISTING_RULE+OFFLINE_FINAL_BLOCK_RECLASS')
  ...
  m=json.loads(mp.read_text(encoding='utf-8'));m['delisting_periods']=periods;m['delisting_first_days']=[{'exchange':p['exchange'],'code':p['code'],'date':p['first_date']} for p in periods];m['data_file']=op.name;m['data_sha256']=sha(op);m['offline_delisting_rule_reclassified']=True
  (out/mp.name).write_text(json.dumps(m,ensure_ascii=False,indent=2),encoding='utf-8');summary['periods']+=periods
 (out/'g4_reclassification.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
 print(json.dumps({'period_count':len(summary['periods']),'rewritten_rows':summary['rewritten_rows'],'regimes':{x:sum(p['regime']==x for p in summary['periods']) for x in sorted({p['regime'] for p in summary['periods']})}},ensure_ascii=False));return 0
