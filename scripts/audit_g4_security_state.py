...
def main():
 ...
 total={'securities':0,'rows':0,'tradable':0,'suspended':0,'risk_warning':0};zeros=[];unknown=[];source_codes=set();source_rows=0;source_hash_missing=0;data_hashes=[];delisting_first_days=[]
 for mp in ms:
  ...
  zeros+=m['zero_source_securities'];unknown+=m['unclassified_special_days'];delisting_first_days+=m.get('delisting_first_days') or []
 ...
 seen=set();state_rows=0;r601268=[];bad_limits=[];row_codes=set();special_no_limit=[]
 ...
 if row_codes!=(expected-set(tuple(z.split(':')) for z in zeros)):
  errors.append(f'row-level security set mismatch row_codes={len(row_codes)} expected_nonzero={len(expected)-len(zeros)}')
