...
import extract_stage3_financial_pdf_values_v19 as prior
from extract_stage3_financial_pdf_values_v6 import declared_a_share_codes
from stage3_financial_pdf_parser_v22 import parse_pdf_bytes as v17_30_parse_pdf_bytes

base = prior.base
METHOD = "CNINFO_ORIGINAL_PDF_PYMUPDF_V20_V17_30_EXACT_SOURCE_CROSS_PAGE_GROUP_EQUITY_PRODUCTION"
METHODOLOGY_VERSION = "V3.3.14-V17.30"
SHARD_GATE = "S3G1J_FINANCIAL_PDF_EXTRACTION_SHARD_V17_30"
RUNTIME_GENERATION = "V17.30"
...
def parse_pdf_bytes(raw: bytes, economic_date: str) -> dict:
    parsed = dict(v17_30_parse_pdf_bytes(raw, economic_date))
...
def _rewrite_manifest(out_root: str, shard: int) -> None:
    ...
    if manifest.get("parser_method") != METHOD:
        raise ValueError(f"V17.30 shard parser method mismatch: {manifest.get('parser_method')}")
    if manifest.get("methodology_version") != METHODOLOGY_VERSION:
        raise ValueError(f"V17.30 shard methodology mismatch: {manifest.get('methodology_version')}")
    if int(manifest.get("shard", -1)) != shard:
        raise ValueError(f"V17.30 shard identity mismatch expected={shard} actual={manifest.get('shard')}")
    manifest["gate"] = SHARD_GATE
    manifest["runtime_generation"] = RUNTIME_GENERATION
    manifest["inactive_runtime_wrapper"] = True
    manifest["runtime_authority_activated"] = False
...
def main() -> int:
    ...
    original_gate = prior.SHARD_GATE
    original_generation = prior.RUNTIME_GENERATION
    prior.parse_pdf_bytes = parse_pdf_bytes
    prior.METHOD = METHOD
    prior.METHODOLOGY_VERSION = METHODOLOGY_VERSION
    prior.SHARD_GATE = SHARD_GATE
    prior.RUNTIME_GENERATION = RUNTIME_GENERATION
    try:
        result = prior.main()
    finally:
        ...
        prior.SHARD_GATE = original_gate
        prior.RUNTIME_GENERATION = original_generation
