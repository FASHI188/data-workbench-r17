...
def _valid_record() -> ExternalDataEnvelope:
    ...
    return ExternalDataEnvelope(
        ...
        event_time=datetime(2026, 8, 31, 0, 0, tzinfo=timezone.utc),
        source="example-official-source",
        source_ref="snapshot://sha256/example",
...
def test_external_envelope_accepts_future_event_announced_earlier() -> None:
    ...
    assert record.event_time > record.published_at
    validate_external_data_envelope(record)
...
def test_external_envelope_rejects_future_information_in_pit_order() -> None:
    ...
    with pytest.raises(ValueError, match="available_at"):
        validate_external_data_envelope(
            replace(record, available_at=record.published_at - timedelta(seconds=1))
        )
    with pytest.raises(ValueError, match="collected_at"):
        validate_external_data_envelope(
            replace(record, collected_at=record.available_at - timedelta(seconds=1))
        )


def test_external_envelope_requires_exact_source_hash() -> None:
    with pytest.raises(ValueError, match="source_sha256"):
        validate_external_data_envelope(replace(_valid_record(), source_sha256="not-a-hash"))
...
def test_extension_default_is_disabled_and_isolated() -> None:
    manifest = ExtensionManifest(
        ...
        module_version="v1",
        contract_version="V1",
        kind=ExtensionKind.SOURCE_ADAPTER,
    )
    validate_extension_manifest(manifest)
    assert manifest.enabled is False
    assert manifest.training_allowed is False
    assert manifest.live_allowed is False
    assert manifest.failure_policy == "ISOLATE_FAIL_CLOSED"
    assert manifest.fallback_behavior == "BASELINE_UNCHANGED"
    ...
    with pytest.raises(ValueError, match="acceptance_ref"):
        validate_extension_manifest(
            ExtensionManifest(
                ...
                module_version="v1",
                contract_version="V1",
                kind=ExtensionKind.FEATURE_PLUGIN,
    ...
    with pytest.raises(ValueError, match="rollback target"):
        validate_extension_manifest(
            ExtensionManifest(
                ...
                lifecycle=ExtensionLifecycle.ENABLED,
                enabled=True,
                acceptance_ref="accepted-evidence",
            )
        )
    ...
    with pytest.raises(ValueError, match="ENABLED"):
        validate_extension_manifest(
            ExtensionManifest(
                ...
                lifecycle=ExtensionLifecycle.ACCEPTED,
                enabled=True,
                acceptance_ref="accepted-evidence",
            )
        )
...
def test_registry_reserves_required_interfaces_but_implements_no_modules() -> None:
    ...
    assert set(registry["required_extension_kinds"]) == {
        "SOURCE_ADAPTER",
        "FEATURE_PLUGIN",
        "MODEL_ADAPTER",
        "TASK_SCHEDULER",
    ...
    assert registry["policies"]["no_speculative_business_logic"] is True
    assert registry["policies"]["failure_must_not_affect_accepted_baseline"] is True
    assert registry["policies"]["shadow_before_enable_required"] is True
    assert registry["policies"]["rollback_required"] is True


def test_empty_module_set_fingerprint_is_deterministic() -> None:
    registry = json.loads((ROOT / "governance" / "extension_module_registry.json").read_text())
    canonical = json.dumps(registry["modules"], sort_keys=True, separators=(",", ":")).encode()
    assert hashlib.sha256(canonical).hexdigest() == registry["module_set_fingerprint"]


def test_registry_locks_current_accepted_baseline_anchor() -> None:
    registry = json.loads((ROOT / "governance" / "extension_module_registry.json").read_text())
    anchor = registry["accepted_baseline_anchor"]
    assert anchor["integration_commit"] == "211b022eb3ff0e355de214a6f83c1eabd6d17b10"
    assert anchor["stage3_dataset_fingerprint"] == "36bfbe0ae703a923ce2575f111a8c68a64c89177c71eeee2cfd9e1ec47bf535f"
...
def test_registry_locks_required_external_lineage_fields() -> None:
    registry = json.loads((ROOT / "governance" / "extension_module_registry.json").read_text())
    required = set(registry["external_data_required_fields"])
    for field in {
        "event_id",
        "idempotency_key",
        "event_time",
...
def test_schemas_are_parseable_and_match_contract_version() -> None:
    external = json.loads(
        (ROOT / "schemas" / "external_data_envelope_v1.schema.json").read_text()
    )
    manifest = json.loads(
        (ROOT / "schemas" / "extension_module_manifest_v1.schema.json").read_text()
    )
    assert external["title"] == "External Data Envelope V1"
    assert manifest["title"] == "Extension Module Manifest V1"
